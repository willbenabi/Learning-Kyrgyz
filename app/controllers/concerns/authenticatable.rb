module Authenticatable
  ...
  def render_unauthorized(message = "Unauthorized")
    # Distinguish between initial page loads, Inertia XHR requests, and JSON API requests
    if request.headers["X-Inertia"] || request.format.json? || request.headers['Accept']&.include?('application/json')
      # Inertia XHR request or JSON API request: Return 401 JSON (frontend handles redirect)
