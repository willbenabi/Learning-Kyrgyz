module Learning
  class ProgressController < ApplicationController
    ...
    before_action :authenticate_user!, only: [:show, :complete_lesson, :add_vocabulary, :update_level, :upgrade_eligibility, :upgrade]

    def index
      # Level 1: Use localStorage data (mock)
      # Level 2: Use real user progress from database

      # Handle JSON API requests (for progressApi.ts getUserProgress)
      # Check Accept header since request.format may not detect JSON correctly
      if (request.format.json? || request.headers['Accept']&.include?('application/json')) && current_user
    ...
    def upgrade_eligibility
      outcome = LevelUpgrade::CheckEligibility.run(user: current_user)
    ...
    def upgrade
      ...
      outcome = LevelUpgrade::Upgrade.run(user: current_user, to_level: to_level)
    ...
    def progress_props(progress)
      {
        ...
        completedLessonIds: {
          grammar: current_user.lesson_completions.by_module('grammar').pluck(:lesson_id),
          reading: current_user.lesson_completions.by_module('reading').pluck(:lesson_id),
          writing: current_user.lesson_completions.by_module('writing').pluck(:lesson_id),
          vocabulary: current_user.lesson_completions.by_module('vocabulary').pluck(:lesson_id)
        },
        stats: {
