...
RSpec.describe UserPolicy, type: :policy do
  ...
  describe "#show?" do
    it "allows super admins to view any user" do
      expect(UserPolicy.new(owner, other_user).show?).to be true
    end
  ...
  describe "#create?" do
    it "grants access to super admins" do
      expect(UserPolicy.new(owner, User).create?).to be true
    end
  ...
  describe "#update?" do
    it "allows super admins to update any user" do
      expect(UserPolicy.new(owner, other_user).update?).to be true
    end
  ...
  describe "#destroy?" do
    it "allows super admins to delete other users" do
      expect(UserPolicy.new(owner, other_user).destroy?).to be true
    end

    it "denies super admins deleting themselves" do
      expect(UserPolicy.new(owner, owner).destroy?).to be false
    end
  ...
  describe "Scope" do
    it "returns all users for super admins" do
      ...
      expect(scope).to include(user1, user2, owner)
    end
  ...
  describe "#permitted_attributes_for_create" do
    it "includes basic user attributes" do
      policy = UserPolicy.new(owner, User.new)
